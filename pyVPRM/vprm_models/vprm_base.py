...
class vprm_base:
    ...
    def get_p_scale(self, lon=None, lat=None,
        ...
        if site_name is not None:
            th = float(self.vprm_pre.min_max_evi.sat_img.sel(site_names=site_name)['th'])
        elif lon is not None:
         #  land_type = self.land_cover_type.value_at_lonlat(lon, lat, key='land_cover_type', interp_method='nearest', as_array=False)
            th = self.vprm_pre.min_max_evi.value_at_lonlat(lon, lat, key='th', interp_method='nearest', as_array=False)
        else:
    ...
    def get_evi(self, lon=None, lat=None, site_name=None):
        ...
        if site_name is not None:
            self.buffer['evi'] =  float(self.vprm_pre.sat_imgs.sat_img.sel(site_names=site_name).isel({self.vprm_pre.time_key : self.vprm_pre.counter})['evi'])
        elif lon is not None:
            self.buffer['evi'] = self.vprm_pre.sat_imgs.value_at_lonlat(lon, lat, as_array=False, key='evi', isel={self.vprm_pre.time_key : self.vprm_pre.counter})
        else:
            self.buffer['evi'] = self.vprm_pre.sat_imgs.sat_img['evi'].isel({self.vprm_pre.time_key : self.vprm_pre.counter})
        return self.buffer['evi']
    ...
    def get_lswi(self, lon=None, lat=None, site_name=None):
        ...
        if site_name is not None:
            self.buffer['lswi'] =  float(self.vprm_pre.sat_imgs.sat_img.sel(site_names=site_name).isel({self.vprm_pre.time_key: self.vprm_pre.counter})['lswi'])
        elif lon is not None:
            self.buffer['lswi'] =  self.vprm_pre.sat_imgs.value_at_lonlat(lon, lat, as_array=False, key='lswi', isel={self.time_key: self.counter})
        else:
            self.buffer['lswi'] = self.vprm_pre.sat_imgs.sat_img['lswi'].isel({self.vprm_pre.time_key: self.vprm_pre.counter})
        return self.buffer['lswi']
    ...
    def _get_vprm_variables(self, land_cover_type, datetime_utc=None, lat=None, lon=None,
        ...
        if len(era_keys) > 0:
            ...
            if (lat is None) & (lon is None):
                    self.era5_inst.regrid(dataset=self.vprm_pre.prototype_lat_lon,
