...
##### Setttings ######
year = 2015
sat_data_bpath = '/work/bd1231/tglauch/one_year_modis_europe_new_{}'.format(year)
copernicus_data_path = '/work/bd1231/tglauch/copernicus_classification'
out_base = '/work/bd1231/tglauch/VPRM_input/'
#####################
...
vprm_inst = vprm()
file_collections = sorted(np.unique([i.split('.')[1] for i in glob.glob(os.path.join(sat_data_bpath, '*h{:02d}v{:02d}*.hdf'.format(18, 3)))]))
for c0, f in enumerate(file_collections):
    print(c0)
    handlers = []
    for c, i in sorted(enumerate(glob.glob(os.path.join(sat_data_bpath, '*{}*.hdf'.format(f))))):
        if c == 0:
    ...
    vprm_inst.add_sat_img(handler, b_nir='B02', b_red='B01',
                          b_blue='B03', b_swir='B06', smearing=False,
                         drop_bands=True) #True) # 

# Sort the satelliges images by time
vprm_inst.sort_and_merge_by_timestamp()


# Load or vreate vegetation map
...
# Do lowess calculation with 120 CPUs
vprm_inst.lowess(n_cpus=120)

#Create output for WRF
out_grid = dict()
out_grid['lons'] = lons
out_grid['lats'] = lats
